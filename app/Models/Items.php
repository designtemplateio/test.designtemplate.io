...
class Items extends Model
{
  ...
  public static function autoSearch($query,$categoryId)
  {
    $value=DB::table('items')->where('item_name', 'LIKE', '%'. $query. '%')->where('item_desc', 'LIKE', '%'. $query. '%')->where('item_shortdesc', 'LIKE', '%'. $query. '%')->where('item_category_parent', 'LIKE', '%'. $categoryId. '%')->where('drop_status','=','no')->where('item_status','=',1)->orderBy('item_name', 'asc')->get(); 
    return $value;
  }
  ...
  public static function getentireItem()
  {
	...
	$setting['setting'] = Settings::editGeneral($sid);
	$site_item_per_page = $setting['setting']->site_item_per_page;
    $value=DB::table('items')->select('items.item_thumbnail','items.item_name','items.item_featured','items.item_token','items.free_download','items.item_flash_request','items.item_flash','items.item_type','items.item_tags','items.item_slug','users.username','items.item_status','items.subscription_item')->join('users','users.id','items.user_id')->where('items.item_status','=',1)->where('items.drop_status','=','no')->orderBy('items.item_id', 'desc')->paginate($site_item_per_page); 
    return $value;
    // ->where('items.item_type','=','Graphics-Design')
// 	->where('items.item_type','!=','Graphics-Design')
  }	
  public static function illustrationItem()
  {
    $sid = 1;
	$setting['setting'] = Settings::editGeneral($sid);
	$site_item_per_page = $setting['setting']->site_item_per_page;
    $value=DB::table('items')->select('items.item_thumbnail','items.item_name','items.item_featured','items.item_token','items.free_download','items.item_flash_request','items.item_flash','items.item_type','items.item_slug','users.username','items.item_status','items.subscription_item')->where('items.item_type','=','Motion-Graphics')->join('users','users.id','items.user_id')->where('items.item_status','=',1)->where('items.drop_status','=','no')->orderBy('items.item_id', 'desc')->paginate($site_item_per_page); 
    return $value;
  }
  ...
  public static function getTrashItem()
  {
    ...
    $value=DB::table('items')->join('users','users.id','items.user_id')->where('items.drop_status','=','yes')->orderBy('items.item_id', 'desc')->get(); 
    return $value;
  ...
  /* Item Tags */
  public static function tagData()
    {
      $value=DB::table('item_tags')->where('item_tags.tag_drop_status','=','no')->orderBy('item_tags.tag_id', 'desc')->get(); 
      return $value;
    
    //   $value = DB::table('item_tags')
	   //  ->where('tag_status', 1)
    //   ->first();
    //   return $value;
    }
    public static function alltagData($nslug)
    {
      $value = DB::table('item_tags')
      ->where('tag_name', $nslug)
      ->where('tag_drop_status', 'no')
	     ->where('tag_status', 1)
      ->get();
      return $value;
    }
    
    public static function deleteItemtag($id,$data){
    ...
    public static function insertItemtags($data)
    {
      DB::table('item_tags')->insert($data);
     
    }	
    
    public static function viewItemtags($tag_id)
    {
    $value = DB::table('item_tags')
      ->where('tag_id', $tag_id)
      ->first();
    ...
    public static function editItemtags($tag_id, $data){
    DB::table('item_tags')
      ->where('tag_id', $tag_id)
      ->update($data);
    }
  ...
  public static function fileDeleted($filename)
  {
	...
	{
	  Storage::disk('wasabi')->delete($file); // wasabi
	  $drop = public_path().'/storage/items/'.$file;  // my server
      File::delete($drop);
	}
  ...
  public static function forceDeleted($token)
  {
		...
		{
		   Storage::disk('wasabi')->delete($file);
		   Storage::disk('wasabi')->delete($file_2);
		   foreach($getall as $get)
			{
				$file_main= $get->item_image;
				Storage::disk('wasabi')->delete($file_main);
			}
		   Storage::disk('wasabi')->delete($image->item_file);
		   Storage::disk('wasabi')->delete($image->video_file);
		   Storage::disk('wasabi')->delete($image->audio_file);
		}
		else if($settings->site_s3_storage == 3)
		{
		   Storage::disk('dropbox')->delete($file);
  ...
  public static function updatecartData($item_id,$session_id,$order_status,$updatedata)
  {
   ...
   public static function updatecartsessionData($old_session_id,$data)
  {
      
    $update = DB::table('item_order')
   ...
   public static function getcartData()
  {
    ...
    $value=DB::table('item_order')
                ->join('users','users.id','item_order.item_user_id')
                ->join('items','items.item_id','item_order.item_id')
                ->where('item_order.session_id','=',$session_id)
                ->where('items.item_status','=',1)
                ->where('items.drop_status','=','no')
                ->where('item_order.order_status','=','pending')
                ->orderBy('item_order.ord_id', 'desc')->get(); 
  ...
  public static function getCoupon($coupon,$user_id)
  {
    ...
    $value=DB::table('coupon')->join('item_order','item_order.item_user_id','coupon.user_id')->where('coupon.coupon_code','=',$coupon)->where('coupon.coupon_status','=',1)->where('item_order.order_status','=','pending')->get();
    return $value;
