...
class Settings extends Model
{
	...
	/* start selling */
	  ...
	  {
	  $today = date('Y-m-d');
	  $get=DB::table('users')->leftJoin('subscription','subscription.subscr_id','users.user_subscr_id')->where('subscription.subscr_status','=',1)->where('subscription.subscr_drop_status','=','no')->get();
	  $value = $get->count(); 
	  return $value;
