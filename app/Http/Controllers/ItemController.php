...
class ItemController extends Controller
{
	...
	public function invoice_download($product_token,$order_id)
	{
		...
		if($check_purchased != 0)
		{
		  ...
		  $pdf_filename = $item['data']->item_slug.'-'.$order_details->ord_id.'-'.$order_details->purchase_token.'.pdf';
        ...
        return  $pdf = PDF::loadView('pdf_view', $data);  
	...
	public function stripe_callback($item_user_id,Request $request)
    {
	    $sid = 1;
	    $setting['setting'] = Settings::editGeneral($sid);
	...
	$mercado_status = $request->input('status');
	$payment_status = 'completed';
	$purchased_token = $ord_token;
	if($mercado_status == 'approved')
	{
		$orderdata = array('payment_token' => $payment_token, 'order_status' => $payment_status);
		$checkoutdata = array('payment_token' => $payment_token, 'payment_status' => $payment_status);
		Items::singleordupdateData($purchased_token,$orderdata);
	...
	public function mercadopago_failure()
	{
	   return view('cancel');
	}
	public function mercadopago_pending()
	{
	   return view('cancel');
	}
	...
	/* stripe */
	public function deposit_stripe_success(Request $request)
    {   
		...
		$response = $this->gateway->confirm([
            'paymentIntentReference' => $request->input('payment_intent'),
            'returnUrl' => $website_url.'/deposit-stripe-success',
        ])->send();
	...
	{
	    $user_id = Auth::user()->id;
	    $free_items = Downloads::checkMonthFreeFiles($user_id);
	    $subscription_items = Downloads::checkMonthSubscription($user_id);
	  ...
	  return view('purchases',[ 'orderData' => $orderData, 'purchase_sale' => $purchase_sale, 'drawal_amount' => $drawal_amount, 'countdata' => $countdata,'free_items' => $free_items, 'subscription_items' => $subscription_items]); 
	 
	}
	
	
	public function downloaded_download($token)
	{
	     if(Auth::check())
	 { 
	...
	{
	   $user_id = Auth::user()->id;
	  $free_items = Downloads::checkauthormyDownload($user_id);
	  $subscription_items = Downloads::checkauthormyDownload($user_id);
	  $downloadData['item'] = Downloads::getuserDownloads();
	  $total_free_download = 0;
	  foreach($downloadData['item'] as $download)
	  {
	    $total_free_download += $download->free_items;
	  }
	  
	   $total_subscription_download = 0;
	  foreach($downloadData['item'] as $download)
	  {
	    $total_subscription_download += $download->subscription_items;
	  }
	  
	  $total_download=$total_free_download + $total_subscription_download;
	  ...
	  return view('sales',[ 'orderData' => $orderData, 'total_sale' => $total_sale, 'purchase_sale' => $purchase_sale, 'credit_amount' => $credit_amount, 'drawal_amount' => $drawal_amount, 'total_free_download' => $total_free_download, 'total_subscription_download' => $total_subscription_download, 'total_download' => $total_download, 'free_items' => $free_items, 'subscription_items' => $subscription_items]); 
	...
	public function show_deposit(Request $request)
	{
	   ...
	   $savedata = array('user_id' => $user_id, 'purchase_token' => $purchase_token, 'payment_type' => $payment_method, 'payment_date' => $payment_date, 'deposit_price' => $final_amount, 'payment_status' => $payment_status);
	   
	   $updatedata = array('payment_type' => $payment_method, 'payment_date' => $payment_date, 'deposit_price' => $final_amount);
