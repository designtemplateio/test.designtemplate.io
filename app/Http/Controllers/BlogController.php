...
class BlogController extends Controller
{
	...
	public function view_single($slug)
	{
	  ...
	  $category_count = Blog::getgrouppostData();
	  return view('blogs', [ 'edit' => $edit, 'slug' => $slug, 'catData' => $catData, 'blog' => $blog, 'blogPost' => $blogPost, 'post_tags' => $post_tags, 'comment' => $comment, 'count' => $count, 'category_count' => $category_count, 'previous' => $previous, 'previous_count' => $previous_count, 'next_count' => $next_count, 'next' => $next]);
	  }
	  else
	  {
	  return redirect('/404');
	...
	public function insert_comment(Request $request)
	{
	   ...
	   {
	      return redirect()->back()->with('error', 'Sorry you are already comment this post.');
	   }
    ...
    //     \Session::put('url.intended', $intendedUrl);
        $data = \Session::put('url.intended', url()->previous());
        echo 1;
	}

	public function checkLoggedOrNot()
	{
	    if(Auth::user()){
	        return 1;
	    }else{
