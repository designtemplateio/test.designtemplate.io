	...
	public function insert_keywords(Request $request)
	{
	   ...
	   $request->validate([
		...
		else
		{
			   ...
			   $token_key = uniqid();
			   $data = array('keyword_id' => $token_key, 'keyword_token' => $keyword_token, 'keyword_label' => $keyword_text, 'keyword_text' => $keyword_text, 'language_code' => $language_code, 'keyword_parent' => $keyword_parent);
			   
			   
			   Languages::saveOnebyOne($data);
			   $lastid = Languages::LastidGet($token_key);
			   $insertedId = $lastid->keyword_id;
			   if($keyword_enable == 1)
			   {
			   	$viewer['all'] = Languages::getSimpleData($language_code);
			   	foreach($viewer['all'] as $views)
			   	{
				   $record = array('keyword_id' => uniqid(), 'keyword_token' => $keyword_token, 'keyword_label' => $keyword_text, 'keyword_text' => $keyword_text, 'language_code' => $views->language_code, 'keyword_parent' => $insertedId);
		...
		else
		{
			   ...
			   if($language_code != 'en')
			   {
				   ...
				   foreach($language['view'] as $translate)
				   {
					   ...
					   $extra_data = array('keyword_id' => uniqid(), 'keyword_token' => $keyword_token, 'keyword_label' => $keyword_label, 'keyword_text' => $keyword_text, 'language_code' => $language_code, 'keyword_parent' => $parent);
