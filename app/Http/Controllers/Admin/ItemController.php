...
class ItemController extends Controller
{
	...
	public function suggestions()
	{
       ...
       return view('admin.suggestions',[ 'commentData' => $commentData]);	
	...
	public static function Image_Path($image_name,$empty_name) 
    {
	   ...
	   else if($allsettings->site_s3_storage == 2)
	   {
		  ...
		  /*if(Storage::disk('wasabi')->exists($image_name))
          {
     ...
     if($request->hasFile('file')) 
	 {
       ...
       if(in_array(strtolower($extension), $validextensions))
	   {
         ...
         //$request->file('file')->move($destinationPath, $fileName);
		       ...
		       $img_name =str_replace(' ', '-', $image_name);
					...
					if($allsettings->site_s3_storage == 1)
				  	{
					  ...
					  else if($allsettings->site_s3_storage == 2)
			  	  	 {
					       ...
					       if($extension == "jpeg" or $extension == "jpg" or $extension == "png" or $extension == "webp")
        					...
        					$img->save(base_path('public/storage/items/'.$img_name),$additional->image_quality);
        					Storage::disk('wasabi')->put($img_name, file_get_contents(public_path('/storage/items/'.$img_name)));
        					File::delete(public_path('/storage/items/'.$img_name));
			  	  		    }
			  	  		    else
			  	  		    {
		 ...
		 $record = '<div class="form-group">
				   ...
				   $record .= '</div>';
		 $record .= '<div class="form-group">
                    <label for="site_desc" class="control-label mb-1">Upload Preview (Size : 700x447px) <span class="require">*</span></label><br/>
	...
	public function view_items()
	{
	  ...
	  $itemData['item'] = Items::getentireItem();
	  $illustration['item'] = Items::illustrationItem();
	  $viewitem['type'] = Items::gettypeItem();
	  $encrypter = app('Illuminate\Contracts\Encryption\Encrypter');
	  $search = "";
	  $data = array('itemData' => $itemData, 'illustration' => $illustration, 'viewitem' => $viewitem, 'encrypter' => $encrypter, 'search' => $search);
	  return view('admin.items')->with($data);
	}
  ...
  /* 	Item Tags */
   public function view_item_tags()
	{
	 
	  $itemData['item'] = Items::tagData();
	  $data = array('itemData' => $itemData);
	  return view('admin.item-tags')->with($data);
	
	}
	public function view_add_item_tags()
	{
	  return view('admin.add-item-tags');
	}
	
	public function view_item_tags_delete($id)
	{
	    $data = array('tag_drop_status'=>'yes');
        
      Items::deleteItemtag($id,$data);
	  
	  return redirect()->back()->with('success', 'Delete successfully.');
	}
	
	
	public function edit_item_tags($id)
	{
	   $edit['item_type'] = Items::viewItemtags($id);
	   $data = array('edit' => $edit);
	   
	   return view('admin.edit-item-tags')->with($data);
	
	}
	
		public function update_edit_item_tags(Request $request)
	{
	
	     $tag_name = $request->input('tag_name');
		 $tag_header_desc = $request->input('tag_header_desc');
		 $tag_footer_desc = $request->input('tag_footer_desc');
		 ...
		 $rules = array(
		        ...
		        'tag_name' => ['required', 'max:255', Rule::unique('item_tags') ->ignore($tag_id, 'tag_id') -> where(function($sql){ $sql->where('tag_drop_status','=','no');})],
		...
		$data = array('tag_name' => $tag_name, 'tag_header_desc' => $tag_header_desc,'tag_footer_desc' => $tag_footer_desc, 'tag_status' => $tag_status);
       
        Items::editItemtags($tag_id, $data);
            
 
       } 
     
       return redirect('/admin/item-tags')->with('success', 'Update successfully.');
	
	}
	
	public function save_add_item_tags(Request $request)
	{
		 ...
		 $rules = array(
				'tag_name' => ['required', 'max:255', Rule::unique('item_tags') -> where(function($sql){ $sql->where('tag_drop_status','=','no');})],
		...
		$data = array('tag_name' => $tag_name, 'tag_header_desc' => $tag_header_desc,'tag_footer_desc' => $tag_footer_desc, 'tag_status' => $tag_status);
        Items::insertItemtags($data);
        
            
 
       } 
     
       return redirect('/admin/item-tags')->with('success', 'Insert successfully.');
  ...
  /* Item Tags */	
	...
	public function subscription_item_request($subscription,$item_token,$free)
	{
...
// 	public function free_item_request($free,$item_token)
// 	{
...
// 	     $subscription_text = 1;
// 	   }
// 	 $data = array('free_download'=> $free_text,'subscription_item'=> $subscription_text);
...
// 	}
    ...
    public function upload_item($itemtype)
    {
		...
		$item_token = "";
		$data = array('getvendor' => $getvendor, 'itemWell' => $itemWell, 'attribute' => $attribute, 'type_id' => $type_id, 'type_name' => $type_name, 'getdata1' => $getdata1, 'getdata2' => $getdata2, 'getdata3' => $getdata3, 'getdata4' => $getdata4, 'getdata5' => $getdata5, 'getdata6' => $getdata6, 'item_token' => $item_token);
	...
	public function update_items(Request $request)
	{
	   ...
	   $previewVideo = $request->input('preview_video');
       if($video = $request->file('preview_video')){
            
	        $preview_video = $video->getClientOriginalName();
	        $path = $video->storeAs('videos', $preview_video);
		...
		else
		{
		 ...
		 $data = array('item_name' => $item_name, 'item_desc' => $item_desc, 'item_thumbnail' => $item_thumbnail, 'item_preview' => $item_preview, 'item_file' => $item_file, 'file_type' => $file_type, 'item_file_link' => $item_file_link, 'item_category' =>$cat_id, 'item_category_parent' => $parent_category_id, 'item_category_type' => $cat_name, 'item_type' => $item_type, 'regular_price' => $regular_price, 'extended_price' => $extended_price, 'demo_url' => $demo_url, 'item_tags' => $item_tags, 'item_status' => $item_status, 'item_shortdesc' => $item_shortdesc, 'free_download' => $free_download, 'item_slug' => $item_slug, 'video_url' => $video_url, 'future_update' => $future_update, 'item_support' => $item_support, 'updated_item' => $updated_item, 'item_flash' => $item_flash, 'video_preview_type' => $video_preview_type, 'video_file' => $video_file, 'item_type_cat_id' => $item_category, 'seller_refund_term' => $seller_refund_term, 'seller_money_back' => $seller_money_back, 'seller_money_back_days' => $seller_money_back_days, 'item_allow_seo' => $item_allow_seo, 'item_seo_keyword' => $item_seo_keyword, 'item_seo_desc' => $item_seo_desc, 'audio_file' => $audio_file, 'item_type_id' => $type_id, 'subscription_item' => $subscription_item, 'preview_video' => $preview_video); 
		   ...
		   if($getvendor['user']->item_review_email == 1)
		   {
            ...
            // return back()->with('success', $item_approve_status);
